...
| ✏️ **이름 자동 정리** | Suno에서 제목을 바꾸면 로컬 파일명도 따라 변경 |
...
**로그인 확인 → 목록 가져오기 → 웹 UI 열기** 까지 자동으로 진행됩니다.
...
`SunoDownloader.exe` 와 `_internal\` 이 만들어집니다.
(`pip install -r requirements.txt` 가 먼저 되어 있어야 합니다.)
...
- **▶ 버튼** — 미리듣기. 받아둔 곡은 로컬에서, 아직 없는 곡은 스트리밍으로 재생
...
flowchart LR
    ...
    E -->|인증 불필요| F["cdn1.suno.ai<br/>실제 오디오 파일"]
    F --> G["wav/ · mp3/"]
    G --> H["downloaded.json<br/>곡 ID 기록"]
```

핵심은 **목록과 파일을 분리**한 점입니다.

Suno의 목록 API는 Clerk 토큰이 필요한데, 이 토큰은 **수명이 60초**입니다.
그래서 목록은 브라우저 세션으로 가져오고, 실제 오디오는 인증이 필요 없는
CDN에서 직접 받습니다. 덕분에 토큰 만료를 신경 쓸 필요가 없습니다.
...
눌러야** 그때 변환됩니다. 그래서 한 번도 받은 적 없는 곡은 CDN에서 `403` 이
돌아옵니다.
...
WAV 자체는 **Pro / Premier 전용**입니다. 무료 플랜은 MP3만 받을 수 있습니다.
</details>
...
- **9월 3일 이후에는 "없으면 WAV 생성"을 끄는 것을 권합니다.** 변환 요청은
  Suno에서 다운로드 버튼을 누르는 것과 같은 동작이라 월 한도에 포함될 수
  있습니다. 이미 받아둔 곡을 다시 받는 것은 CDN에서 가져오므로 한도와
  무관합니다.
